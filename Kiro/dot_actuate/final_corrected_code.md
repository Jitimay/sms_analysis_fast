Here is the complete and corrected code. This version includes the `modem.enableSSL()` command, which should resolve the "400 Bad Request" error.

```cpp
// ===============================
// TTGO T-Call SIM800L + Moonbeam RPC Test
// Corrected for latest TinyGSM library + SSL enabled
// ===============================
...
void loop() {
  SerialMon.print("\nEnabling SSL...");
  // 1. Enable SSL before connecting
  if (!modem.enableSSL()) {
    SerialMon.println(" fail");
    delay(10000);
    return;
  ...
  // 2. Disable SSL after the session
  SerialMon.print("Disabling SSL...");
  if (!modem.disableSSL()) {
    SerialMon.println(" fail");
  } else {
    SerialMon.println(" success");
