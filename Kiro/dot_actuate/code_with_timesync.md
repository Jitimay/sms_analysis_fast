This version adds the crucial step of synchronizing the device's time with the cellular network before attempting an SSL connection.

```cpp
// ===============================
// TTGO T-Call SIM800L + Moonbeam RPC Test
// Final version with Time Sync and Root CA Certificate
// ===============================
...
void setup() {
  ...
  // =================================================
  // CRITICAL STEP: Synchronize time for SSL validation
  // =================================================
  ...
  if (modem.getNetworkTime(&year, &month, &day, &hour, &minute, &second, &timezone)) {
    ...
    SerialMon.print("Current time set to: ");
    SerialMon.print(asctime(&t));

  } else {
    SerialMon.println(" fail. SSL connection will likely fail.");
  }
  // =================================================
}
